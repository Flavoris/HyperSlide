//
//  AttackModeEffect.swift
//  HyprGlide
//
//  Lightweight state container for temporary attack mode bonuses.
//

import Foundation
import CoreGraphics

/// Tracks the lifetime of an attack mode effect granted by power-ups.
struct AttackModeEffect {
    
    private(set) var remaining: TimeInterval = 0
    private let duration: TimeInterval
    private let maxStackDuration: TimeInterval?
    
    /// Indicates whether attack mode is currently active.
    var isActive: Bool {
        remaining > 0
    }
    ...
    /// Clears any residual attack mode time.
    mutating func reset() {
        remaining = 0
    }
}
